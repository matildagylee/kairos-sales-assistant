...
- **Status:** v1 shipped on the `output-quality` branch.

## 2026-07-31 — Output v2 (on `output-quality`)
- **Expand depth:** tapping a chip shows the **top 3** points (kept, not trimmed to 2).
- **Auto-tightened bullets:** each battlecard now stores short versions (`sayThis`, `whyWeWinShort`, `theirWeaknessShort`, `discoveryShort`, ~9-14 words) generated by AI from the Notion source. The card shows these; full wording stays in Notion. Regenerated on "resync battlecards".
- **Speed:** copilot sends Claude only the *scoped* knowledge base (in-play competitor + selected persona + vertical proof points + a compact list of other competitors), not all 9 cards. Copilot answers capped at 400 output tokens. Default model changed to **Sonnet 4.6** (fast + accurate) for live-call latency; Opus/Haiku still selectable.
- **Relevance:** when call notes are recording, the tail of the live transcript is fed into the copilot so answers reflect what's being said.
- **Trust:** hardened rule — every claim must come from the knowledge base and cite its source; if not covered, say "Not in the battlecard" instead of guessing.
